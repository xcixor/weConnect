...
class TestUserCase(unittest.TestCase):
    ...
    def test_valid_username(self):
        ...
        response = self.invalid_username_user.register_user(self.user_list)
        self.assertEqual(response['message'], 
        'Username cannot contain special characters')

    def test_valid_email(self):
        """Test app cannot allow invalid email"""
        response = self.invalid_email_user.register_user(self.user_list)
    ...
    def test_valid_password_length(self):
        ...
        response = self.short_password_user.register_user(self.user_list)
    ...
    def test_login(self):
        ...
        self.success_user.register_user(self.user_list)
        response  = self.success_user.login(self.user_list, 'ptah', 'pass123')
        self.assertTrue(response)

    def test_logout(self):
        """Test user can logout successfully"""
        self.success_user.register_user(self.user_list)
        self.success_user.login(self.user_list, 'ptah', 'pass123')
    ...
    def test_reset_password(self):
        ...
        self.success_user.register_user(self.user_list)
        result = User.reset_password(self.user_list, self.success_user.name,'pass123', 
        '123pass')
        self.assertEqual(result['message'], 'Password successfully changed')

    def test_reset_password_with_wrong_previous_password(self):
        self.success_user.register_user(self.user_list)
        result = User.reset_password(self.user_list, self.success_user.name, 'wrong345', 
        'new1234')
