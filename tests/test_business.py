...
from app.api_v1.models import Business, Review

class TestBusinessCase(unittest.TestCase):
    """Test Business class functionality
    Attributes:
        business(obj): a business object to be used for subsequent tests
    Methods:
    ...
    """
    def setUp(self):
        self.mock_business = Business("James Barber", "We sell bananas", "Kawangware", "fruit vendors")
    ...
    def test_create_business(self):
        ...
        self.assertEqual(Business.get_all_businesses(), [{"Name":"James Barber", "Descrition":"We sell bananas", "Location":"Kawangware", "Category":"fruit vendors"}])
    
    def test_cannot_create_duplicate_businesses(self):
        """Test app cannot duplicate business"""
        self.mock_business.create_business()
        another_biz = Business("james barber", "We sell bananas", "Kawangware", "fruit vendors")
        response = another_biz.create_business()
        self.assertEqual(response['message'], \
        'Cannot create duplicate business')

    def test_edit_business(self):
        """Test app can edit a business successfully"""
        self.mock_business.edit_business()
        response = self.mock_business.edit_business(Description='We no longer sell bananas')
        self.assertTrue(response)
        self.assertEqual(Business.get_all_businesses(), [{"Name":"James Barber", "Description":"We no longer sell bananas", "Location":"Kawangware", "Category":"fruit vendors"}])
    ...
    def test_find_event_by_id(self):
        ...
        self.assertEqual(response, {"Name":"James Barber", "Descrition":"We sell bananas", "Location":"Kawangware", "Category":"fruit vendors"})

    def test_find_businesses_by_category(self):
        """Test app can find a businesses by category"""
        self.mock_business.create_business()
        another_biz = Business("jane's fruits", "All kinds of fruits", "Kihanya", "fruit vendors")
        another_biz.create_business()
        response = Business.get_businesses_by_category()
        self.assertEqual(response, [{"Name":"James Barber", "Description":"We sell bananas", "Location":"Kawangware", "Category":"fruit vendors"},
                        {"Name":"jane's fruits", "Description":"All kinds of fruits", "Location":"Kihanya", "Category":"fruit vendors"}])

    def test_find_businesses_by_location(self):
        """Test app can find a businesses by category"""
        self.mock_business.create_business()
        another_biz = Business("juma cafe", "Mandazi moto stop", "Kawangware", "food")
        another_biz.create_business()
        response = Business.get_businesses_by_category()
        self.assertEqual(response, [{"Name":"James Barber", "Description":"We sell bananas", "Location":"Kawangware", "Category":"fruit vendors"},
                        {"Name":"juma cafe", "Description":"Mandazi moto stop", "Location":"Kawangware", "Category":"food"}])
        

    def test_add_business_review(self):
        """Test user can write a business review"""
        self.mock_business.create_business()
        response = self.mock_business.write_review('Loved the place', self.success_user.username)
    ...
    def test_get_all_reviews(self):
        ...
        self.assertEqual(response, [{'id':'1', 'comment':'Worst services ever'}, {'id':'2', 'comment':'Impunity of the highest order'}])